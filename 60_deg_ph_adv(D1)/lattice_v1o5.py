...
# Ring layout:
# - 3 arcs × (2 suppressor cells + 14 arc cells + 2 suppressor cells) = 54 fodo cells
# - straight section ~ 25 m, full lattice length ~ 345.0 m
...
env = xt.Environment()
env.particle_ref = xt.Particles(kinetic_energy0=2.86e9, mass0 = xp.ELECTRON_MASS_EV)            # 2.86 GeV energy
env.vars.default_to_zero = True

#Physical dimensions of quads and bends

env['l_quad'] = 0.45   #quadrupole length  (all quadrupoles are the same length for now)
env['l_dipole'] = 1.5  #bending length
env['ang_mb'] = 2* np.pi /96  #bending angle (96 total bending magnets)
env['l_fodo'] = 5.00435  #length of each FODO cell 


#ARC FODO CELL SETUP

#  Drift values based on 5.00435 m per cell  
...
# element definitions (drifts, quads, bends)
...
opt_tune = arc_cell_comp.match(
    ...
    targets=[
        xt.TargetSet(qx=1/6, qy=1/6, tol=1e-6),
    ]
)
...
# %%
#additional quadrupole strengths (for suppressor and striaght regions)
...
# element definitions of the above quads
...
env['d_sup3'] = 2 + (env['l_fodo'] - 2 * env['l_dipole'] - 1.5 * env['l_quad']) / 4  
...
env['d_between_quads_triplet'] = 0.25 # [m] distance between given 2 quads in the straight section
...
# now to place wigglers in the drifts of some straight cells(12/15 in the full ring), we also define a new drift that is dr_str - total wiggler length
...
suppressor_cell_1 = env.new_line(components=[
    ...
    env.place('dr_sup1') ,
    env.place('qd_sup_b') , env.place('dr_sup1') ,
...
suppressor_cell_2 = env.new_line(components=[
    ...
    env.place('dr_sup2') , env.place('mb') ,
    env.place('dr_sup2') ,
    env.place('qd_sup_a') , env.place('dr_sup2'),env.place('mb') ,
    env.place('dr_sup2') , env.place('half_qf_sup')
])
...
#1/6th of the ring modelled as a "transfer line"
...
arc_twiss_init = arc_twiss.get_twiss_init(at_element=0)     # for initial twiss from the arc

#straight region 'doublet' initial guesses
env['KQF_str_doublet'] = 1.5
env['KQD_str_doublet'] = -1.1
...
#looking at the full table to play around with the matching location
#display(HTML(tw_transfer_df.to_html()))

# Optionally, export to CSV:
#tw_transfer_df.to_csv('twiss_table_C2.csv')
...
opt_disp_TL = transfer_line_one_sixth_ring.match(
    ...
    vary=[
        ...
        xt.Vary('kqf_sup',  step=1e-4),                                                                            #still a free parameter, can be used
    ...
    targets=[
        ...
        xt.Target(bety=3.0, at='wig1_long_5.straight_wig1.triplet_2_0', tol=1e-6),  
        xt.Target(betx=3.0, at='wig1_long_5.straight_wig1.triplet_2_0', tol=1e-6),
        xt.Target(bety=3.0, at='dr_str_triplet_slice_63.triplet_2_0::0', tol=1e-6),
        xt.Target(betx=3.0, at='dr_str_triplet_slice_63.triplet_2_0::0', tol=1e-6),
    ]
)
...
# %%
# some rematching, this time taking mux and muy instead of the beta funtions in the straight section with no wigglers
...
opt_disp_TL = transfer_line_one_sixth_ring.match(
    ...
    targets=[
        ...
        xt.Target(alfx=0.0, at='_end_point', tol=1e-6),
        xt.Target(alfy=0.0, at='_end_point', tol=1e-6),
        xt.Target(bety=3.5, at='wig1_long_5.straight_wig1.triplet_2_0', tol=1e-3),  
        xt.Target(betx=3.5, at='wig1_long_5.straight_wig1.triplet_2_0', tol=1e-3),
        xt.Target(mux = 11.7304/6, at='_end_point', tol=1e-4),                         # 11.7304 (say) is the total phase advance in the ring, divided by 6 for the transfer line
        xt.Target(muy = 11.783/6, at='_end_point', tol=1e-4),                          # 11.783 (say) is the total phase advance in the ring, divided by 6 for the transfer line
    ]
)
...
twiss_ring_plot.ylim(left_lo=0,left_hi=18, right_lo=-0.2, right_hi=1,
...
# Display the full scrollable table inside your notebook:
#display(HTML(twiss_RING_df.to_html()))

# Optionally, export to CSV:
#twiss_RING_df.to_csv('twiss_table.csv')


# %%
#print("revolution period : ", twiss_RING.T_rev0)
...
opt_chroma = RING.match(
    ...
    method='6d',
    vary=[
        xt.Vary('k2_sf', step=1e-3, limits=(-100, 100)),
        xt.Vary('k2_sd', step=1e-3, limits=(-100, 100)),
    ],
    targets=[
...
# %%
#export as json
RING.to_json('lattice_v1o5.json')
